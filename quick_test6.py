"""Quick test 6: Showcase the EasyRanker meta-wrapper class in Remote Cloud Mode.

This script demonstrates forcing EasyRanker to use the remote Jina AI Cloud API,
leveraging large sentence chunk context limits (3000 characters).

Usage:
    python3 quick_test6.py
"""
...
print("=" * 80)
print("EASYRANKER DEMO (FORCED REMOTE JINA CLOUD MODE)")
print("=" * 80)
print()

try:
    # 2. Initialize forcing remote backend
    print("Step 1: Initializing EasyRanker explicitly in 'remote' mode targeting Jina...")
    ranker = EasyRanker(backend='remote', model='jina-reranker-v3')
    ...
    # Note that we are leveraging Jina Remote's massive 131K token context capability by 
    # setting a large sentence length limit (3000 characters) without server size crashes!
    results_dir = ranker.rerank(
        query="Separation of religious institutions from civil government authority",
        documents=madison_dir,
        top_n=3,
        verbose=True,
        max_sentence_length=3000  # Leverage Jina Remote's massive context (131K tokens)
    )
    ...
    print("\n" + "=" * 80)
    print("EasyRanker remote demo successfully completed!")
    print("=" * 80)
